...
class FluxConfig(BaseModel):
    ...
    trt_image_height: Optional[int] = None
    trt_image_width: Optional[int] = None
...
class FluxGenerator:
    def __init__(self, config: FluxConfig):
        ...
        # Compare the provided config name with the available configs
        if config.name not in configs:
            available = ", ".join(configs.keys())
            raise ValueError(f"Got unknown model name: {config.name}, chose from {available}")

        if not config.trt:
            self.t5 = load_t5(self.device, max_length=256 if config.name == "flux-schnell" else 512)
            ...
            self.ae = engines[ModuleName.VAE].to(device="cpu" if offload else self.device)
            self.model = engines[ModuleName.TRANSFORMER].to(device="cpu" if offload else self.device)
            self.clip = engines[ModuleName.CLIP].to(self.device)
            self.t5 = engines[ModuleName.T5].to(device="cpu" if offload else self.device)
    ...
    def __call__(self, args: FluxArgs):
        ...
        if self.offload:
            ...
            torch.cuda.empty_cache()
            self.ae.decoder = self.ae.decoder.to(x.device)  # Need to reassign!
